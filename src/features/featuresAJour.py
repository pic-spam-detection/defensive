...
sample_features = sample_emails.apply(
...
features_df["email_complet"] = (
    sample_emails["subject"] + " " + sample_emails["body"]
)  # Ajouter l'email complet
features_df["is_spam"] = features_df.apply(
    evaluer_spam, axis=1
)  # Évaluer si c'est un spam

# Réorganiser les colonnes pour affichage
features_df = features_df[
    ["email_complet", "is_spam"]
    + [col for col in features_df.columns if col not in ["email_complet", "is_spam"]]
]
...
def extraire_features(subject, body):
    ...
    tfidf_score = (
        np.mean(tfidf_vectorizer.fit_transform([mail]).toarray())
        if nombre_mots > 0
        else 0
    )
...
train_features = train_df.progress_apply(
    lambda row: extraire_features(row["subject"], row["body"]), axis=1
)
