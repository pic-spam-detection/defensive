...
class Vote(BaseModel):
    def __init__(self, models_type: List[BaseModel] = MODELS, checkpoint_path: Optional[str] = None, threshold: float = 0.5):
        super().__init__()
        self.models = [ClassicalMLClassifier(classifier_type=model, checkpoint_path=checkpoint_path) for model in models_type]
    ...
    def train(self, dataset: List[Dict[str, str]], save_path: Optional[str] = None) -> Dict[str, BaseModel]:
